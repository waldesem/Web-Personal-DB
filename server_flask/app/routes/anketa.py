...
class AnketaView(MethodView):
    ...
    def post(self, item):
        ...
        if item == "resume":
            ...
            person_id = self.upload(json_data) if json_data else None
            ...
            person_id = self.upload(anketa.pop("resume"))
            if not person_id:
...
views = AnketaView.as_view("anketa")
bp.add_url_rule("/<item>", view_func=views)
