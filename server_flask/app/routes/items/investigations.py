...
from ...model.models import Investigation
from ...model.tables import Investigations, db_session
...
class InvestigationsView(MethodView):
    ...
    def get(self, item_id):
        ...
        """
        stmt = select(Investigations).filter(Investigations.person_id == item_id)
        query = db_session.execute(stmt.order_by(desc(Investigations.id)))
        return jsonify([row._asdict() for row in query]), 200

    @validate()
    @roles_required(Roles.user.value)
    def post(self, item_id, json_data: Investigation):
        """
        ...
        if item_id:
            item = db_session.get(Investigations, item_id)
            for key, value in json_dict.items():
                setattr(item, key, value)
        else:
            db_session.add(Investigations(**json_dict))
        db_session.commit()
    ...
    def delete(self, item_id):
        ...
        Returns:
            ...
            code of 204.
        """
        table = db_session.get(Investigations, item_id)
