...
from app.model.models import Staff
from app.model.tables import Staffs, db_session
...
class StaffView(MethodView):
    ...
    def post(self, item_id: int, json_data: Staff) -> Response:
        """Insert or replac a record in the specified table with the given item ID.

        Args:
            item_id (int): The ID of the record to insert or replace.
            json_data (Staff): The data to insert or replace.
        ...
        if item_id:
            item = db_session.get(Staffs, item_id)
            for key, value in json_dict.items():
                setattr(item, key, value)
        else:
            table = Staffs(**json_dict)
    ...
    def delete(self, item_id: int) -> Response:
        ...
        """
        table = db_session.get(Staffs, item_id)
