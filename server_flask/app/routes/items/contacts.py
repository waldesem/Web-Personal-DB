...
from ...model.tables import Contacts, db_session
...
class ContactsView(MethodView):
    ...
    def get(self, item_id):
        ...
        """
        stmt = select(Contacts).filter(Contacts.person_id == item_id)
        query = db_session.execute(stmt.order_by(desc(Contacts.id))).scalars()
        return jsonify([row.to_dict() for row in query]), 200

    @validate()
    @roles_required(Roles.user.value)
    def post(self, item_id, json_data: Contact):
        """
        ...
        if item_id:
            item = db_session.get(Contacts, item_id)
            for key, value in json_dict.items():
                setattr(item, key, value)
        else:
            db_session.add(Contacts(**json_dict))
        db_session.commit()
    ...
    def delete(self, item_id):
        ...
        Returns:
            ...
            code of 204.
        """
        table = db_session.get(Contacts, item_id)
