...
from ...model.models import Poligraf
from ...model.tables import Poligrafs, db_session
...
class PoligrafView(MethodView):
    ...
    def get(self, item_id):
        ...
        """
        stmt = select(Poligrafs).filter(Poligrafs.person_id == item_id)
        query = db_session.execute(stmt.order_by(desc(Poligrafs.id)))
        return jsonify([row._asdict() for row in query]), 200

    @validate()
    @roles_required(Roles.user.value)
    def post(self, item_id, json_data: Poligraf):
        """
        ...
        if item_id:
            ...
            db_session.add(Poligrafs(**json_dict))
        db_session.commit()
    ...
    def delete(self, item_id):
        ...
        Returns:
            ...
            code of 204.
        """
        table = db_session.get(Poligrafs, item_id)
