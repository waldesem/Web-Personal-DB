...
from app.model.models import Inquiry
from app.model.tables import Inquiries, db_session
...
class InquiriesView(MethodView):
    ...
    def get(self, item_id: int) -> Response:
        ...
        """
        stmt = select(Inquiries).filter_by(person_id=item_id)
        query = db_session.execute(stmt.order_by(desc(Inquiries.id))).scalars()
        return jsonify([row.to_dict() for row in query]), 200

    @validate()
    @roles_required(Roles.user.value)
    def post(self, item_id: int, json_data: Inquiry) -> Response:
        ...
        Args:
            ...
            json_data (Inquiry): The data to insert or replace.
        ...
        item = Inquiries(**json_dict, person_id=item_id, user_id=current_user.id)
    ...
    def delete(self, item_id: int) -> Response:
        ...
        """
        stmt = text("DELETE FROM inquiries WHERE person_id = :item_id")
