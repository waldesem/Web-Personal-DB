...
from ...model.models import Education
from ...model.tables import Educations, db_session
...
class EducationView(MethodView):
    ...
    def get(self, item_id):
        ...
        query = db_session.execute(stmt.order_by(desc(Educations.id)))
        return jsonify([row._asdict() for row in query]), 200

    @validate()
    @roles_required(Roles.user.value)
    def post(self, item_id, json_data: Education):
        """
