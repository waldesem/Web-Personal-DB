...
from app.model.models import Affilation
from app.model.tables import Affilations, db_session

bp = Blueprint("affilations", __name__, url_prefix="/affilations")


class AffilationsView(MethodView):
    """Affilation routes."""
    ...
    def get(self, item_id: int) -> Response:
        ...
        """
        stmt = select(Affilations).filter(Affilations.person_id == item_id)
        query = db_session.execute(stmt.order_by(desc(Affilations.id))).scalars()
        return jsonify([row.to_dict() for row in query]), 200

    @validate()
    @roles_required(Roles.user.value)
    def post(self, item_id: int, json_data: Affilation) -> Response:
        ...
        Args:
            ...
            json_data (Affilation): The data to insert or replace.
        ...
        if item_id:
            item = db_session.get(Affilations, item_id)
            for key, value in json_dict.items():
                setattr(item, key, value)
        else:
            db_session.add(Affilations(**json_dict))
        db_session.commit()
    ...
    def delete(self, item_id: int) -> Response:
        ...
        """
        table = db_session.get(Affilations, item_id)
        db_session.delete(table)
        db_session.commit()
        return jsonify({"message": "success"}), 201


bp.add_url_rule("/<int:item_id>", view_func=AffilationsView.as_view("affilation"))
