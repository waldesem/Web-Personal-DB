...
class AffilationsView(MethodView):
    ...
    def get(self, item_id: int) -> Response:
        ...
        """
        stmt = select(Affilations).filter_by(person_id=item_id)
    ...
    def post(self, item_id: int, json_data: Affilation) -> Response:
        ...
        item = Affilations(**json_dict, person_id=item_id) # deprecated: , user_id=current_user.id)
    ...
    def delete(self, item_id: int) -> Response:
        ...
        """
        stmt = text("DELETE FROM affilations WHERE id = :item_id")
