...
async function mainPage(path) {
    ...
    async function tableView(path, currentPage = 1) {
        ...
        ul.addEventListener("click", function switchPage(event) {
            ...
            if (parentTarget.isEqualNode(nextPage) && has_next !== 0)
                tableView(path, currentPage + 1);
            if (parentTarget.isEqualNode(prevPage) && has_prev !== 0)
                tableView(path, currentPage - 1);
        }, { once: true });
    }
    ;
 ...
 */
function createCheck(candId, check, state, status, checkId) {
    ...
    if (status != state['NEWFAG'] && status !== state['UPDATE']) {
 ...
 */
async function checkEditNew(candId, flag, checkId, check = null) {
 ...
 */
function createRegistry(candId, registry, state, status, registryId) {
 ...
 */
function createPoligraf(candId, poligraf, poligrafId) {
 ...
 */
function createInvestigation(candId, investigation, investigationId) {
 ...
 */
function createInquiry(candId, inquiry, inquiryId) {
 ...
 */
function createTabAction(candId, parentDiv, formHTML, path) {
    // Get the "add" button element. Add a click listener to the "add" button
    const addBtnListener = parentDiv.children[1];
    addBtnListener.addEventListener('click', function (event) {
        ...
        const form = parentDiv.children[0].children[0];
        form.addEventListener('submit', async function (event) {
