...
class MainApp {
  ...
  appFooter = document.createElement('div');
   ...
   */
  createMessage(styling: string, text: string): void{
...
function userLogin(
  ...
  if (path === "/login") {
    ...
    anchor.onclick = function () {userLogin(
      '/password',
      "alert-primary", 
  ...
  login.addEventListener('submit', async function submitForm (event: any) {
...
function createAnketa(
  ...
  if (anketa[0][0]["status" as keyof typeof anketa[0][0]] != state['NEWFAG' as keyof typeof state] && 
...
async function statInfo(flag=false): Promise<void> {
  ...
  if (!flag) {
    ...
    [candStat, pfoStat, formStat].forEach((stat, i) => {
      ...
      if (i == 2) stat.innerHTML = formInfo;
  ...
  const form = appMain.appContent.children[2].children[0] as HTMLFormElement;
