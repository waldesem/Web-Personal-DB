...
<template>
  <UFormGroup :state="checkForm" class="mb-3" size="lg" label="Негатива нет">
    <UToggle v-model="noNegative"/>
  </UFormGroup>
  <UForm @submit.prevent="submitCheck">
    <UFormGroup class="mb-3" size="lg" label="Проверка по местам работы">
      <UTextarea
        v-model="checkForm['workplace']"
        placeholder="Проверка по местам работы"
      />
    </UFormGroup>
    <UFormGroup class="mb-3" size="lg" label="Проверка документов">
      <UTextarea
        v-model="checkForm['document']"
        placeholder="Проверка документов"
      />
    </UFormGroup>
    <UFormGroup class="mb-3" size="lg" label="Проверка ИНН">
      <UTextarea
        v-model="checkForm['inn']"
        placeholder="Проверка ИНН"
      />
    </UFormGroup>
    <UFormGroup class="mb-3" size="lg" label="Проверка задолженностей">
      <UTextarea
        v-model="checkForm['debt']"
        placeholder="Проверка задолженностей"
      />
    </UFormGroup>
    <UFormGroup class="mb-3" size="lg" label="Проверка банкротства">
      <UTextarea
        v-model="checkForm['bankruptcy']"
        placeholder="Проверка банкротства"
      />
    </UFormGroup>
    <UFormGroup class="mb-3" size="lg" label="Проверка Кредитной истории">
      <UTextarea
        v-model="checkForm['bki']"
        placeholder="Проверка Кредитной истории"
      />
    </UFormGroup>
    <UFormGroup class="mb-3" size="lg" label="Проверка судебных дел">
      <UTextarea
        v-model="checkForm['courts']"
        placeholder="Проверка судебных дел"
      />
    </UFormGroup>
    <UFormGroup class="mb-3" size="lg" label="Проверка аффилированности">
      <UTextarea
        v-model="checkForm['affilation']"
        placeholder="Проверка аффилированности"
      />
    </UFormGroup>
    <UFormGroup class="mb-3" size="lg" label="Проверка в списке террористов">
      <UTextarea
        v-model="checkForm['terrorist']"
        placeholder="Проверка в списке террористов"
      />
    </UFormGroup>
    <UFormGroup class="mb-3" size="lg" label="Проверка в розыск">
      <UTextarea
        v-model="checkForm['mvd']"
        placeholder="Проверка в розыск"
      />
    </UFormGroup>
    <UFormGroup class="mb-3" size="lg" label="Проверка в открытых источниках">
      <UTextarea
        v-model="checkForm['internet']"
        placeholder="Проверка в открытых источниках"
      />
    </UFormGroup>
    <UFormGroup class="mb-3" size="lg" label="Проверка в Кронос">
      <UTextarea
        v-model="checkForm['cronos']"
        placeholder="Проверка в Кронос/Крос"
      />
    </UFormGroup>
    <UFormGroup class="mb-3" size="lg" label="Комментарии">
      <UTextarea
        v-model="checkForm['comment']"
        placeholder="Комментарии"
      />
    </UFormGroup>
    <UFormGroup class="mb-3" size="lg" label="Результат">
      <USelect
