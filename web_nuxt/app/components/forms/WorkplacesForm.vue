...
form.value.starts = form.value.starts ? useDateFormat(form.value.starts, "YYYY-MM-DD").value : "";
...
<template>
  <UForm :state="form" @submit.prevent="emit('update', form)">
    ...
    <UFormField v-if="!form.now_work" label="Начало работы" name="starts" required>
    ...
    <UFormField label="Должность" name="position" required>
      <UInput v-model.trim.lazy="form.position" placeholder="Должность" maxlength="255" required />
    </UFormField>
    <UFormField label="Адрес организации" name="address">
      <UTextarea v-model.trim.lazy="form.address" placeholder="Адрес организации" />
    </UFormField>
    <UFormField label="Причина увольнения" name="reason">
      <UTextarea v-model.trim.lazy="form.reason" placeholder="Причина увольнения" />
    </UFormField>
    <ElementSubmitButton />
  </UForm>
</template>
