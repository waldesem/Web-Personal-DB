...
const { refresh, status } = await useLazyAsyncData("staffs", async () => {
  staffs.value = await useFetch("/api/items/staffs/" + props.candId) as Staff[];
});

async function submitStaff(form: Staff) {
  closeAction();  
  pending.value = true;
  const { message } = await useFetch(`/api/items/staffs/${props.candId}`, {
    method: "POST",
    body: form,
...
async function deleteStaff(id: string) {
  ...
  const { message } = (await useFetch(`/api/items/staffs/${id}`, {
    method: "DELETE",
  })) as Record<string, string>;
  await refresh();  
  emit("message", message);
