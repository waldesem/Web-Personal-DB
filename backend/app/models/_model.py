...
class User(Base):
    ...
    messages = db.relationship(
        "Message", backref="users", cascade="all, delete, delete-orphan"
    )
    roles = db.relationship(
        "Role", secondary=user_roles, backref=db.backref("users", lazy="dynamic")
    )
    groups = db.relationship(
        "Group", secondary=user_groups, backref=db.backref("users", lazy="dynamic")
    )
...
class Person(Base):
    ...
    documents = db.relationship(
        "Document", backref="persons", cascade="all, delete, delete-orphan"
    )
    addresses = db.relationship(
        "Address", backref="persons", cascade="all, delete, delete-orphan"
    )
    workplaces = db.relationship(
        "Workplace", backref="persons", cascade="all, delete, delete-orphan"
    )
    contacts = db.relationship(
        "Contact", backref="persons", cascade="all, delete, delete-orphan"
    )
    staffs = db.relationship(
        "Staff", backref="persons", cascade="all, delete, delete-orphan"
    )
    checks = db.relationship(
        "Check", backref="persons", cascade="all, delete, delete-orphan"
    )
    robots = db.relationship(
        "Robot", backref="persons", cascade="all, delete, delete-orphan"
    )
    poligrafs = db.relationship(
        "Poligraf", backref="persons", cascade="all, delete, delete-orphan"
    )
    inquiries = db.relationship(
        "Inquiry", backref="persons", cascade="all, delete, delete-orphan"
    )
    investigations = db.relationship(
        "Investigation", backref="persons", cascade="all, delete, delete-orphan"
    )
    relations = db.relationship(
        "Relation", backref="persons", cascade="all, delete, delete-orphan"
    )
    affilations = db.relationship(
        "Affilation", backref="persons", cascade="all, delete, delete-orphan"
    )
