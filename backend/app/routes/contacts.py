...
from . import bp
from ..models.model import db, Connect, Group
from ..models.schema import ConnectSchema


@bp.route('/connects/<group>/<flag>/<int:page>', methods=['GET', 'POST'])
@jwt_required()
@bp.doc(hide=True)
...
@bp.route('/contact/<action>/<int:item_id>', methods=['GET', 'POST'])
@jwt_required()
@bp.doc(hide=True)
def update_contact(action, item_id):
    if action == 'delete':
        ...
        json_data = {k: v for k, v in json_data if k != k['id'] or k != k['data']}
