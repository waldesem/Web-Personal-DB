...
class PersonView(MethodView):
    ...
    def get(self, person_id):
        ...
        return {
            item: query
            for item, query in zip(
                items, [view.get("view", person_id) for view in views]
            )
        }
    ...
    def get(self, action, person_id):
        ...
        if action == "status":
            ...
            if person.has_status(
                Statuses.new.value, Statuses.update.value, Statuses.repeat.value
            ):
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        db.session.add(
                            Check(officer=current_user.fullname, person_id=person_id)
                        )
                        db.session.commit()
...
bp.add_url_rule(
    "/resume/<action>/<int:person_id>", view_func=resume_view, methods=["GET", "DELETE"]
)
...
bp.add_url_rule("/staff/<action>/<int:item_id>", view_func=StaffView.as_view("staff"))
...
bp.add_url_rule(
    "/document/<action>/<int:item_id>", view_func=DocumentView.as_view("document")
)
...
bp.add_url_rule(
    "/address/<action>/<int:item_id>", view_func=AddressView.as_view("address")
)
...
bp.add_url_rule(
    "/contact/<action>/<int:item_id>", view_func=ContactView.as_view("contact")
)
...
bp.add_url_rule(
    "/workplace/<action>/<int:item_id>", view_func=WorkplaceView.as_view("workplace")
)
...
bp.add_url_rule(
    "/relation/<action>/<int:item_id>", view_func=RelationView.as_view("relation")
)
...
bp.add_url_rule(
    "/affilation/<action>/<int:item_id>", view_func=AffilationView.as_view("affilation")
)
    ...
    def get(self, action, item_id):
        if action == "add":
            ...
            if person.has_status(
                ...
                db.session.add(Check(officer=current_user.fullname, person_id=item_id))
                db.session.commit()
                return "", 201
        ...
        if action == "self":
            ...
            db.session.add(
                Message(
                    message=f"Aнкета ID #{id} делегирована " f"{current_user.fullname}",
            ...
            db.session.add(
                Message(
                    message=f"Aнкета ID #{id} переделегирована "
...
bp.add_url_rule("/check/<action>/<int:item_id>", view_func=CheckView.as_view("check"))
...
bp.add_url_rule(
    "/investigation/<action>/<int:item_id>",
...
bp.add_url_rule(
    "/poligraf/<action>/<int:item_id>", view_func=PoligrafView.as_view("poligraf")
)
...
bp.add_url_rule(
    "/inquiry/<action>/<int:item_id>", view_func=InquiryView.as_view("inquiry")
)
