...
from ..models.model import db, User, Role, Message
from ..models.schema import UserSchema, MessageSchema
...
class MessageView(MethodView):
    ...
    def delete(self, item_id):
        ...
        if not item_id:
            messages = (
                db.session.execute(select(Message).filter_by(user_id=current_user.id))
                .scalars()
                .all()
            )
            if len(messages):
