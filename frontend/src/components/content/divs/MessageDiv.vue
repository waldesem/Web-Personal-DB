...
async function deleteMessage(event: Event, iD: number = 0): Promise<void> {
  ...
  try {
    const response = await axiosAuth.delete(`${server}/messages/${iD}`);
...
<template>
  <a href="#offcanvasMessage" role="button" data-bs-toggle="offcanvas">
    <i
      class="fs-3"
      :class="`${
        stateMessage.messages.length ? 'bi bi-bell-fill' : 'bi bi-bell'
      }`"
    >
    </i>
    <span
  ...
  >
    <div class="offcanvas-body">
      ...
      <div
        v-for="(message, index) in stateMessage.messages"
        :key="index"
        class="card mb-3"
      >
